...
def test_area():
    ...
    assert area(r) == pytest.approx(expected_area, rel=1e-9)  # Проверка с точностью до 1e-9

# Тест для функции perimeter
def test_perimeter():
    r = 3
    expected_perimeter = 2 * math.pi * r
    assert perimeter(r) == pytest.approx(expected_perimeter, rel=1e-9)  # Проверка с точностью до 1e-9
