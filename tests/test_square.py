...
def test_area():
    ...
    expected_area = a * a
    assert area(a) == expected_area  # Проверяем, что возвращаемое значение совпадает с ожидаемым

# Тест для функции perimeter
def test_perimeter():
    a = 4  # Пример стороны квадрата
    expected_perimeter = 4 * a
    assert perimeter(a) == expected_perimeter  # Проверяем, что возвращаемое значение совпадает с ожидаемым
